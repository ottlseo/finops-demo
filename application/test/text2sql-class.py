...
class GraphHandler():
    def __init__(self):
        ...
        # LLM model settings
        self.llm_model = "anthropic.claude-3-5-sonnet-20241022-v2:0"
        ...
        self.json_response_format = """'The output should be formatted as a JSON instance that conforms to the JSON schema below.\n\nAs an example, for the schema {"properties": {"foo": {"title": "Foo", "description": "a list of strings", "type": "array", "items": {"type": "string"}}}, "required": ["foo"]}\nthe object {"foo": ["bar", "baz"]} is a well-formatted instance of the schema. The object {"properties": {"foo": ["bar", "baz"]}} is not well-formatted.\n\nHere is the output schema:\n```\n{"properties": {"setup": {"title": "Setup", "description": "question to set up a joke", "type": "string"}, "punchline": {"title": "Punchline", "description": "answer to resolve the joke", "type": "string"}}, "required": ["setup", "punchline"]}\n```'"""
        
        # Database settings
        self.engine = create_engine("sqlite:///Chinook.db")
        self.db = SQLDatabase(self.engine)
        self.DIALECT = "sqlite"
        self.Session = sessionmaker(bind=self.engine)
        
        # Initialize clients
        self.boto3_client = self.init_boto3_client()
        self.sql_search_client, self.table_search_client, self.sql_retriever, self.table_retriever = self.init_search_resources()
    ...
    def init_boto3_client(self):
        retry_config = Config(
            region_name=self.region_name,
            retries={"max_attempts": 10, "mode": "standard"}
        )
        return boto3.client("bedrock-runtime", region_name=self.region_name, config=retry_config)

    def init_search_resources(self):
        ...
        sql_retriever = OpenSearchVectorRetriever(sql_search_client, region_name=self.region_name, k=20)
        table_retriever = OpenSearchVectorRetriever(table_search_client, region_name=self.region_name, k=10)
    ...
    def get_column_description(self, table_name):
        query = {
            "query": {
                "match": {
        ...
        response = self.table_search_client.conn.search(index=self.table_search_client.index_name, body=query)
    ...
    def search_by_keywords(self, keyword):
        ...
        response = self.table_search_client.conn.search(
            index=self.table_search_client.index_name,
            body=query
        )
    ...
    def analyze_intent(self, state: GraphState) -> GraphState:
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, question=question)
        intent = self.converse_with_bedrock(sys_prompt, usr_prompt)
        
        return GraphState(intent=intent)

    def get_sample_queries(self, state: GraphState) -> GraphState:
        question = state["question"]
        samples = self.sql_retriever.vector_search(question)
        page_contents = [json.loads(doc.page_content) for doc in samples]

        bedrock_agent_runtime = boto3.client('bedrock-agent-runtime', region_name=self.region_name)
        rerank_model_id = "cohere.rerank-v3-5:0"
        model_package_arn = f"arn:aws:bedrock:{self.region_name}::foundation-model/{rerank_model_id}"
    ...
    def check_readiness(self, state: GraphState) -> GraphState:
        print(state)
        question = state["question"]
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, question=question, sample_queries=sample_queries, table_details=table_details)
        readiness = self.converse_with_bedrock(sys_prompt, usr_prompt)
        
        return GraphState(readiness=readiness)

    def get_relevant_tables(self, state: GraphState) -> GraphState:
        question = state["question"]
        tables = self.table_retriever.vector_search(question)
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, question=question, table_inputs=table_inputs, csv_list_response_format=self.csv_list_response_format)
        selected_tables = self.converse_with_bedrock(sys_prompt, usr_prompt)
    ...
    def describe_schema(self, state: GraphState) -> GraphState:
        ...
        for table_name in table_names:
            ...
            table_desc = self.get_column_description(table_name)
    ...
    def next_step_by_intent(self, state: GraphState) -> GraphState:
        return state["intent"]

    def next_step_by_readiness(self, state: GraphState) -> GraphState:
        return state["readiness"]

    ################## SubGraph 2) Text2SQL ##################
    def generate_query(self, state: GraphState) -> GraphState:
        new_query_state = copy.deepcopy(self.initial_query_state)
        question = state["question"]
        sample_queries = state["sample_queries"]
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
            question=question, dialect=self.DIALECT, sample_queries=sample_queries, 
            table_details=table_details, hint=hint)
        generated_query = self.converse_with_bedrock(sys_prompt, usr_prompt)
    ...
    def validate_query(self, state: GraphState) -> GraphState:
        ...
        if self.DIALECT.lower() not in explain_statements:
            query_plan = " "
        else:
            try:
                explain_query = explain_statements[self.DIALECT.lower()].format(query=query)
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
            question=question, dialect=self.DIALECT, query=query, query_plan=query_plan)
        validated_query = self.converse_with_bedrock(sys_prompt, usr_prompt)
    ...
    def handle_failure(self, state: GraphState) -> GraphState:
        query_state = copy.deepcopy(state["query_state"])
        query = query_state['query']
...
Skip the preamble and only provide the valid JSON document."""
        ...
        result = self.converse_with_bedrock(sys_prompt, usr_prompt)
    ...
    def get_relevant_columns(self, state: GraphState) -> GraphState:
        query_state = copy.deepcopy(state["query_state"])
        question = state["question"]
        query = query_state["query"]
        message = query_state['error']['message']
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
            question=question, query=query, message=message, 
            csv_list_response_format=self.csv_list_response_format)
        keywords = self.converse_with_bedrock(sys_prompt, usr_prompt)
        return keywords

    def next_step_by_query_state(self, state: GraphState) -> GraphState:
        return state["query_state"]["status"]

    def next_step_by_next_action(self, state: GraphState) -> GraphState:
        return state["next_action"]

    ################## Answer generation nodes ##################

    def get_general_answer(self, state: GraphState) -> GraphState:
        question = state["question"]
        ...
        sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
            question=question)
        answer = self.converse_with_bedrock(sys_prompt, usr_prompt)

        return GraphState(answer=answer)

    def get_database_answer(self, state: GraphState) -> GraphState:
        question = state["question"]
        ...
        if query_state["status"] == "success":
            ...
            sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
                question=question, query=query, data=data)
        else:
            ...
            sys_prompt, usr_prompt = self.create_prompt(sys_prompt_template, usr_prompt_template, 
                question=question, query=query, failed_step=failed_step, message=message)    
